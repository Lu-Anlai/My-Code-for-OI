...
namespace SegmentTree{
	...
	inline bool queryAns(reg int L,reg int R){
		if(L>R)
			return false;
		Node info=query(1,1,n,L,R);
		reg int len=R-L+1;
		if(info.Max-info.Min!=len-1)
			return false;
		reg int sum_real=mul(add(info.Max%p,info.Min%p),mul(len,inv2_p));
		if(sum_real!=info.sum)
			return false;
		reg int a_0=((info.Min-1)%p+p)%p;
		reg int part1=mul(len,mul(a_0,a_0));
		reg int part2=mul(add(a_0,a_0),sigma1(len));
		reg int part3=sigma2(len);
...
int main(void){
	...
	while(m--){
		...
		switch(op){
			...
			case 2:{
				l=read(),r=read();
				reg bool ans=SegmentTree::queryAns(l,r);
				puts(ans?"damushen":"yuanxing");
				break;
			}
		}
	}
