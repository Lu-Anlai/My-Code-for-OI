#include<bits/stdc++.h>
using namespace std;
#define reg register
typedef long long ll;
#define getchar() (p1==p2&&(p2=(p1=buf)+fread(buf,1,100000,stdin),p1==p2)?EOF:*p1++)
static char buf[100000],*p1=buf,*p2=buf;
#define flush() (fwrite(wbuf,1,wp1,stdout),wp1=0)
#define putchar(c) (wp1==wp2&&(flush(),0),wbuf[wp1++]=c)
static char wbuf[1<<21];int wp1;const int wp2=1<<21;
inline int read(void){
	reg char ch=getchar();
	reg int res=0;
	while(!isdigit(ch))ch=getchar();
	while(isdigit(ch))res=10*res+(ch^'0'),ch=getchar();
	return res;
}

inline void write(reg int x){
	static char buf[32];
	reg int p=-1;
	if(x==0) putchar('0');
	else while(x) buf[++p]=(x%10)^'0',x/=10;
	while(~p) putchar(buf[p--]);
	return;
}

namespace Poly{
	const int p=1e9+9;
	inline int add(reg int a,reg int b){
		a+=b;
		return a>=p?a-p:a;
	}
	inline int sub(reg int a,reg int b){
		a-=b;
		return a<0?a+p:a;
	}
	typedef vector<int> poly;
	inline void FWT_or(poly& a,reg int flag){
		reg int n=a.size();
		for(reg int i=1;i<n;i<<=1)
			for(reg int j=0;j<n;j+=(i<<1))
				for(reg int k=0;k<i;++k)
					if(flag==1)
						a[i+j+k]=add(a[i+j+k],a[j+k]);
					else
						a[i+j+k]=sub(a[i+j+k],a[j+k]);
		return;
	}
}

using namespace Poly;

int main(void){
	reg int n=read(),S=(1<<n);
	poly a[n+1],b[n+1],c[n+1];
	for(reg int i=0;i<=n;++i)
		a[i].resize(S),b[i].resize(S),c[i].resize(S);
	for(reg int i=0;i<S;++i)
		a[__builtin_popcount(i)][i]=read();
	for(reg int i=0;i<S;++i)
		b[__builtin_popcount(i)][i]=read();
	for(reg int i=0;i<=n;++i)
		FWT_or(a[i],1),FWT_or(b[i],1);
	for(reg int i=0;i<=n;++i)
		for(reg int j=0;i+j<=n;++j)
			for(reg int k=0;k<S;++k)
				c[i+j][k]=add(c[i+j][k],1ll*a[i][k]*b[j][k]%p);
	for(reg int i=0;i<=n;++i)
		FWT_or(c[i],-1);
	for(reg int i=0;i<S;++i)
		write(c[__builtin_popcount(i)][i]),putchar(i==S-1?'\n':' ');
	flush();
	return 0;
}