...
int cnt=1,head[MAXN],to[(MAXN*MAXM+2*MAXN+MAXN)<<1],w[(MAXN*MAXM+2*MAXN+MAXN)<<1],Next[(MAXN*MAXM+2*MAXN+MAXN)<<1];
...
inline void Work(void){
	for(reg int i=1;i<=n;++i)
		if(flow[i]<0)
			Add_Tube(i,Tt,-flow[i]);
		else
			Add_Tube(Ts,i,flow[i]);
	Add_Tube(t,s,INF);
	Dinic(Ts,Tt);
	reg int ans=w[cnt];
	w[cnt]=w[cnt-1]=0;
